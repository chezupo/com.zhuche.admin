...
const TableRender: React.FC = () => {
  ...
  const handleReject = (item: TransactionItemType, reason: string) => {
    setLoading(true)
    rejectWithdraw(item.id, reason).then(newItem => {
      setData({
        ...data,
  ...
  const handleConfirm = (item: TransactionItemType) => {
    setLoading(true)
    accessWithdraw(item.id).then(newItem => {
      setData({
        ...data,
        list: data.list.map(i => i.id === newItem.id ? newItem : i)
      })
            ...
            <Row gutter={[0, 24]}>
              <Col span={12}>
                <Popconfirm
                  ...
                  title={`您正在向用户的支付宝账号进行转账。同意后，将会从您的支付宝账号向会向用户的支付宝账户转账${Math.abs(record.amount).toFixed(2)}元，且操作成功后，不可撤消。是否确定操作?`}
                  okText='确定'
                  cancelText='取消'
                >
              ...
              <Col span={12}>
                <CancelButtonRender onCancel={reason => handleReject(record, reason)}/>
              </Col>
            </Row>
          )
        } else {
